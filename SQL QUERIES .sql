...
#count of sensor data oer day
SELECT date, COUNT(*) AS count_per_date FROM md_data   GROUP BY Date order by date;


#month wise machine failure count 
SELECT Date, Machine_ID, COUNT(*) AS count_per_machine
FROM md_data
WHERE downtime = "machine_failure" 
AND substr(Date,1,2) IN (2, 3, 4)  -- Filtering for March, April, Feb
...
select sum(downtime_num) from md_data where  `Spindle_Vibration(Âµm)` >0 and  `Spindle_Vibration(Âµm)` <2;
...
#number of machine failures in diffrent ranges
SELECT 
...
#number of no_failure events in diffrent ranges
SELECT 
    COUNT(*) AS total_failures,
    SUM(CASE WHEN `Hydraulic_Pressure(bar)` BETWEEN -14.326454 AND 101.184167 THEN 1 ELSE 0 END) AS hydraulic_pressure_failures,
    SUM(CASE WHEN `Coolant_Pressure(bar)` BETWEEN 0.325 AND 4.955532 THEN 1 ELSE 0 END) AS coolant_pressure_failures,
...
SELECT 
    ...
    SUM(CASE WHEN `Hydraulic_Pressure(bar)` BETWEEN 50 AND 70 THEN 1 ELSE 0 END) AS in_range_50_to_70,
    SUM(CASE WHEN `Hydraulic_Pressure(bar)` BETWEEN 70 AND 100 THEN 1 ELSE 0 END) AS in_range_70_to_100,
...
-- finding optimum operating range for spindle speed 

SELECT 
    count(*) as total_failures_spindle_speed,
    SUM(CASE WHEN `Spindle_Speed(RPM)` BETWEEN 0 AND 20000 THEN 1 ELSE 0 END) AS in_range_0_to_20000,
    SUM(CASE WHEN `Spindle_Speed(RPM)` BETWEEN 20000 AND 25000 THEN 1 ELSE 0 END) AS in_range_20000_to_25000,
    SUM(CASE WHEN `Spindle_Speed(RPM)` BETWEEN 25000 AND 35000 THEN 1 ELSE 0 END) AS in_range_25000_to_35000
FROM md_data as total_data
